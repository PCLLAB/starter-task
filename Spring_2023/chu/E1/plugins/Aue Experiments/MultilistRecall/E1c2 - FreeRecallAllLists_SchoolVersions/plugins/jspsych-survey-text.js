...
jsPsych.plugins['survey-text'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      // add text box
      $("#jspsych-survey-text-" + i).append('<textarea autofocus name="#jspsych-survey-text-response-' + i + '" cols="' + trial.columns[i] + '" rows="' + trial.rows[i] + '"></textarea>');
    }

    $("#jspsych-survey-text-response-0").focus();
