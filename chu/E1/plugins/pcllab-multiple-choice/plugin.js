...
jsPsych.plugins["pcllab-multiple-choice"] = (function () {
    ...
    plugin.trial = function (display_element, trial) {
        ...
        trial.insert_break = trial.insert_break || []
        ...
        function loadQuestion() {
            ...
            var currQuestion = currStimulus

            var currOptions = trial.options

            if (typeof currStimulus === 'object') {
                //found object, must have answers in form answer1, ..., answerN
                let index = 1
                currOptions = []
                currQuestion = currStimulus.question
                while (currStimulus['answer' + index]) {
                    currOptions.push(currStimulus['answer' + index])
                    index++
                }
                console.log("current options", currOptions)
            }
            ...
            // Stimulus.
            var stimulus = $('<h3 class="text-center mb-2 mt-2">' + currQuestion + '</h3>');
            ...
            for (var i = 0; i < currOptions.length; i++) {
                ...
                if (trial.multiple_choices) {
                    option = $("<div class='checkbox md-checkbox'><input name='mc' id='mc-" + i + "' type='checkbox'><label for='mc-" + i + "'>" + currOptions[i] + "</label></div>");
                } else {
                    option = $("<div class='checkbox md-radio'><input name='mc' id='mc-" + i + "' type='radio'><label for='mc-" + i + "'>" + currOptions[i] + "</label></div>");
                }

                options.append(option);

                if (trial.horizontal && trial.insert_break.indexOf(i) == -1) {
            ...
            if (trial.multiple_choices && trial.none_of_the_above) {
                const notaCheckbox = $("<input name='mc' id='cb-nota' type='checkbox'>");
                const notaLabel = $("<label for='cb-nota'>" + (trial.none_of_the_above || "None of the above") + "</label>");
                noneOfTheAbove = $("<div class='checkbox md-checkbox pcllab-mc-noneoftheabove'></div>");
                noneOfTheAbove.append(notaCheckbox);
                noneOfTheAbove.append(notaLabel);
                options.append(noneOfTheAbove);

                notaCheckbox.click(() => {
                    const children = options.find('input')

                    if (notaCheckbox.is(":checked")) {
                        // Uncheck and disable other checkboxes because the only option should be NOTA
                        for (let i = 0; i < children.length; i++) {
                            const child = $(children[i])
                            if (child.attr('id') !== notaCheckbox.attr('id')) {
